...
ControlPointPlot::ControlPointPlot(QWidget *parent) : QWidget(parent)
{
	...
	whichControlPoint = 0;	// First control point
   ...
   if(nCP == 0)
	...
	if (iter == 1) {
      for (i=0; i<nCP; i++) {
         ...
         controlPointPaths[i][0].moveTo(iter, -u);  // Minuses for the y component is used because in widget coordinates +y axis points downwards.
