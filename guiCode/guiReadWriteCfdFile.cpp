...
void mainWindow::readCfdFile()
{
	...
	// Cuneyt: The following control seems to be unnecessary because getOpenFileName() does not allow the selection of a non-existing file.

   // if (cfdFile.fail()) {
	//	appendMessage(tr("ERROR: Unable to open the cfd file"), Qt::red);
	//	return;
	//}

	// Cuneyt: Multi-block support of the following code is not checked.
   ...
   if (dummyInt > 0) problem->mesh->allocateBlocks();
	...
	cfdFile >> dummyInt;		// Mesh generation algorithm (0: TFI Linear, 1: ... //cuneyt: burayi tamamla)
   ...
   if (problem->mesh->getIsMeshGenerated()) {
		for (k=0; k<problem->mesh->getnBlocks(); k++) {
			...
			// cuneyt: Asagidaki 3 satirin yeri esasinda burasi degil.
...
void mainWindow::writeCfdFile()
{
	...
	float *dummyCoor;
	ofstream cfdFile, defaultFile;

	cfdFile.open(problem->getCfdFileName().c_str(), ios::out|ios::trunc);  // Cuneyt: If not opened, give a warning.
	
	// Cuneyt: Multi-blcok support of the following code is not checked.

	// Write the file header
	cfdFile << "This file is created by Virtual Flow Lab 0.1. It may get corrupt if you modify it outside Virtual Flow Lab." << "\n";
	...
	for (p = 0; p < MAX_PRIMITIVES; p++) {
      if (problem->mesh->primitives[p].getIsDeleted())  continue;
		...
		if (problem->mesh->primitives[p].getType() == LINE) {
			...
			dummyCoor = problem->mesh->primitives[p].getEnteredPointCoor();  // cuneyt: Burada bir problem var mi?
	...
	cfdFile << left << setw(3) << problem->getMeshGenerationAlgorithm() << "\n";	// 0: TFI Linear, 1: ... //cuneyt: burayi tamamla

	// cuneyt: TFI'dan baska ag yaratma algoritmasi yazilinca min iter, max iter, tolerance, weight factor degerleri buraya konacak.
   ...
   if (problem->mesh->getIsMeshGenerated()) {
	...
	// Now we can create the DEFAULT.txt file that the solver needs. It only has the problem name in it.
	defaultFile.open(problem->getDefaultFileName().c_str(), ios::out|ios::trunc);
	defaultFile << problem->getName();
	defaultFile.close();
...
void mainWindow::writeInputFile()
{
   ...
   //if (! problem->mesh->getIsMeshGenerated()) {  // Exit if there is no mesh
	...
	// Write the file header
	inputFile << "This file is created by Virtual Flow Lab 0.1. It may get corrupt if you modify it outside Virtual Flow Lab." << "\n";
	...
	for (b = 0; b < problem->mesh->getnBlocks(); b++) {
		...
		inputFile << "0.0" << "\n";   // cuneyt: Bu son yazialn scalar degisken icin IC bilgisi. Bu su anda eksik ve hep sifir olarak yaziliyor.
	}

	// Write SECTION PARAMETERS
	// Cuneyt: Asagidaki bazi parametreler dogrudan arayuzdeki textbox'lardan alinmaya calisiliyor. Eger texbox bos ise birsey yazilamiyor.
	// Bu sonradan .inp dosyasini okurken sikinti yaratiyor. O yuzden de bos ise rastgele bir deger yaziliyor. Buna daha iyi bir cozum lazim.
	inputFile << "SECTION PARAMETERS********************************" << "\n";
	inputFile << "17 \n";  // cuneyt:   problem->getNparameters() << "\n";
	inputFile << "Scheme                                  " << schemeCombo->currentIndex() + 1 << "\n";			// Cuneyt: Buradaki ve alttaki +1'ler olmamali. Esasinda cozucudekiler de sifirdan baslamali.
