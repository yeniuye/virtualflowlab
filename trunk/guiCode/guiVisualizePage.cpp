...
void mainWindow::openResultFile()
{
   ...
   QString fileName = QFileDialog::getOpenFileName(this, tr("Open Result File"), dir, tr("Problem Files (*.dat)"));
	...
	if (!fileName.isEmpty()){
		...
		problem->setResultFileName(dir + "/" + baseName + ".dat");   // e.g. C:\MyProblems/TestProblem_100.dat
...
}  // End of readResultFile()


/*
void mainWindow::calculateContourData()
{
	// cuneyt: Bu fonksiyonun multi-block destegi yok.


}  // End of calculateContourData()
*/
...
void mainWindow::showContoursButtonClicked()
{
   contourCheckBox->setChecked(true);
	glWidget->update();
}  // End of showContoursButtonClicked()



void mainWindow::clearAllControlPoints()
{
	// cuneyt: Multi-block destegi yok.
	problem->mesh->blocks[0].setnControlPoints(0);
	glWidget->update();
}
...
void mainWindow::clearAllBlockedCells()
{
	...
	problem->mesh->blocks[0].setnBlockedCells(0);
	glWidget->update();
}
...
}  // End of visualizeButtonsClicked()
...
void mainWindow::probeAtPosition(float x, float y)
{
	...
	b = 0;  // cuneyt: Multi-block destegi eksik
	nX = problem->mesh->blocks[b].getnXpoints();
	nY = problem->mesh->blocks[b].getnYpoints();
	...
	// cuneyt: Eger (x,y) blocked cell icinde ise isCellFound FALSE yapilabilir.
	...
	if (cell > -1 && !problem->mesh->blocks[b].isCellBlocked[cell]) {	// If a non-blocked cell is found
		...
		// cuneyt: Burada da hucreler Kartezyen kabul ediliyor.
...
}  // End of probeRadioButtonsToggled()
...
//		}
		...
		distance = stepSize * min(fabs(cornerCoor[1][0]-cornerCoor[0][0]), fabs(cornerCoor[2][1]-cornerCoor[0][1]));	// cuneyt: Hucreler Kartezyen kabul ediliyor.
...
}  // End of placeStreamlineRadioButtonsToggled()
...
void mainWindow::placeStreamlineButtonClicked()
{
   ...
   streamlineCheckBox->setChecked(true);
	glWidget->update();
}  // End of placeStreamlineButtonClicked()
