// This is the implementation file structuredMesh.cpp
...
void StructuredMesh::updateMinMaxUsed()
{
	...
	for (int i = 0; i < MAX_PRIMITIVES; i++) {
		...
		dummyCoor = primitives[i].getDefPointCoor();  // cuneyt: Burada bir problem var mi?
...
int StructuredMesh::isLineValid(float* &coor)
{
	...
	for (int i = 0; i < MAX_PRIMITIVES; i++) {   // Check if a line the same as this one already exists or not.
		...
		dummyCoor = primitives[i].getEnteredPointCoor();  // cuneyt: Burada bir problem var mi?
...
}  // End of calculateNumberOfBlocks
...
}  // End of calculateNumberOfPrimitivesOnFaces




void StructuredMesh::allocateFaceData()   // cuneyt: Bu fonksiyon burada olmamali. Block'un altina tasinabilir.
{
...
}  // End of calculateNumberOfPointsOnFaces
...
{
	// cuneyt: Bu fonksiyon cok cok karisik. Ben bile anlamiyorum artik icini.
	// Sadece kontrol yapilmiyor, kontrollerden gecilirse mesela setOrderedList gibi seyler de yapiliyor.
	// Bunlar farkli fonksiyonlara tasinmali.
  ...
  //int nPrimitives = getnPrimitives();   // shortcut for number of total primitives
  ...
  for (i = 0; i < nBlocks; i++) {
    for (j = 0; j < 4; j++) {
	  if (blocks[i].faces[j].getnPrimitives() == 0) {
        return FALSE;   // cuneyt:  block i, face j tanimlanmamis seklinde bir mesaj verilebilir.
      }
    }
  }
...
void StructuredMesh::generateLinearTFIMesh()
{
	...
	*/
	...
	for (b = 0; b < nBlocks; b++) {
		...
		delete[] blocks[b].coordinates;						// cuneyt: bunu kontrol et
		blocks[b].coordinates = new float[2*sMax*tMax];

		// Allocate blocked cell array. cuneyt: Bunun esas yeri burasi degil.
		...
		south = new float[sMax][2];					// cuneyt: bunlarin boyutlarini degistir.
		...
		dummy = NULL;
		delete[] dummy;					// cuneyt: bunu kontrol et.
